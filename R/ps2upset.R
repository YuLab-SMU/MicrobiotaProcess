...
#' @examples
#' data(test_otu_data)
#' upsetda <- ps2upset(test_otu_data, factorNames="group")
...
#' @aliases ps2upset,phyloseq
#' @rdname ps2upset
#' @export
setMethod("ps2upset", "phyloseq", function(obj,...){
    otuda <- checkotu(obj)
    sampledata <- checksample(obj)
    daupset <- ps2upset(obj=otuda, sampleda=sampledata,...)
    return(daupset)
})
